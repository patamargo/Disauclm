...
result = prom.custom_query(query='up{job="my-service"}')
print(result)
from prometheus_client import CollectorRegistry, Gauge, push_to_gateway

registry = CollectorRegistry()
g = Gauge('custom_metric_from_jupyter', 'Processed in Jupyter', registry=registry)
g.set(42.0)

push_to_gateway('localhost:9091', job='jupyter_job', registry=registry)
...
def evaluate_model():
    # Simulación de precisión de modelo
    acc = random.uniform(0.7, 0.99)  # Sustituir por la precisión real
    model_accuracy.set(acc)

if __name__ == "__main__":
    start_http_server(8000)    # Exponemos el endpoint en localhost:8000/metrics
    while True:
